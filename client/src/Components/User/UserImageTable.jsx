...
function UserImageTable({
  ...
  useEffect(() => {
    ...
    const connectToWebSocket = () => {
        const socket = io("http://localhost:8000", {path: "/job_socket", withCredentials: true});
              ...
              {Object.entries(image).map(([key, img], index) => (
                <Table.Row key={index}>
                  ...
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 flex flex-row gap-2 items-center">
                    <Avatar
                      size="xs"
                      img="https://storage.googleapis.com/corn_sight_public/test_thumbnail.jpg"
                    />
                    <Label>{key}</Label>
                  </Table.Cell>
                  ...
                  <Table.Cell>{new Date(img.upload_date).toLocaleDateString(undefined, {month: 'long', day: 'numeric', year: 'numeric'})}</Table.Cell>
                  <Table.Cell>
                    <BsThreeDotsVertical />
                  </Table.Cell>
                </Table.Row>
